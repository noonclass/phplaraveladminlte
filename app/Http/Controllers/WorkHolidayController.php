...
class 	WorkHolidayController extends Controller
{
    ...
    {
         $whilelist_number = 'whilelist_number';
         $blacklist_number='blacklist_number';
        if (Auth::user()->hasRole('admin')){
            //$result = Tenantmeta::all();             
            $result = DB::table('tenantmeta')->where('tenantmeta.meta_key','<>',$whilelist_number)->where('tenantmeta.meta_key','<>',$blacklist_number)
                    ->leftJoin('tenants', 'tenantmeta.tenant_id', '=', 'tenants.id')
                    ->select('tenantmeta.*','tenants.name')
                    ->get();
        }elseif(Auth::user()->hasRole('moderator')){
            $tenant_id = Auth::user()->tenant_id;
            $result = DB::table('tenantmeta')->where('tenantmeta.tenant_id', $tenant_id)->where('tenantmeta.meta_key','<>',$whilelist_number)->where('tenantmeta.meta_key','<>',$blacklist_number)
                    ->leftJoin('tenants', 'tenantmeta.tenant_id', '=', 'tenants.id')
                    ->select('tenantmeta.*','tenants.name')
                    ->get();
		}
        else{}
        return view('work_holiday')->with('data',$result);
    }
    ...
    {
        $meta_key = $request->changemeta_key;
        $tenantId = $request->tenantId;
        if (Auth::user()->hasRole('admin')){
            //$result = Tenantmeta::all();   
            $result = DB::table('tenantmeta')->where('tenantmeta.meta_key',$meta_key)->where('tenantmeta.tenant_id',$tenantId)
                    ->leftJoin('tenants', 'tenantmeta.tenant_id', '=', 'tenants.id')
                    ->select('tenantmeta.*','tenants.name')
                    ->get();
        }elseif(Auth::user()->hasRole('moderator')){
            $tenant_id = Auth::user()->tenant_id;
            $result = DB::table('tenantmeta')->where('tenantmeta.tenant_id', $tenant_id)->where('tenantmeta.meta_key',$meta_key)
                    ->leftJoin('tenants', 'tenantmeta.tenant_id', '=', 'tenants.id')
                    ->select('tenantmeta.*','tenants.name')
                    ->get();
		}
